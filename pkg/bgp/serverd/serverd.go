...
func (server *BgpServer) StopServer() {
	server.bgpServer.StopBgp(context.Background(), &api.StopBgpRequest{})
}
...
func (server *BgpServer) ReconcileRoutes(ip string, prefix uint32, nexthops []string) (err error) {
	var (
		toAdd    []string
		toDelete []string
	)
