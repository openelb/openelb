...
var _ = Describe("Ip test", func() {
	...
	It("Should caculate correct ip address", func() {
		...
		Expect(GetValidAddressCount("192.168.0.1~192.168.0.10")).Should(Equal(10))
		Expect(GetValidAddressCount("10.0.1.4~10.0.1.255")).Should(Equal(251))
		Expect(GetValidAddressCount("10.0.1.4~10.0.2.3")).Should(Equal(254))
		Expect(GetValidAddressCount("172.16.0.0~172.31.255.255")).Should(Equal(1<<20 - 16*256*2))
	})
})
